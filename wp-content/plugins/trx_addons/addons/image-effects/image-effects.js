(function(){"use strict";var planeClassPrefix='trx_addons_image_effects_on_';var globalCanvas=false;var permanentDrawing=globalCanvas||false;var curtains=null;var $document=jQuery(document);var firstLoad=false;var sliderInited=false;window.addEventListener('load',function(){if(typeof trx_addons_apply_filters=='function'){globalCanvas=trx_addons_apply_filters('trx_addons_filter_image_effects_use_global_canvas',globalCanvas);permanentDrawing=globalCanvas||false;}
firstLoad=true;create_planes();});function create_planes(){if(typeof window.elementorFrontend!=='undefined'&&elementorFrontend.isEditMode()){return;}
...
+(jQuery('body').hasClass('allow_lazy_load')?'.lazyload_inited':''));if(planeElements.length===0)return;if(globalCanvas){curtains=create_canvas(document.body);}
...
parent=img.parentNode;if(img_all.length==1){parent.classList.add('trx_addons_image_effects_holder');}
img.setAttribute('data-sampler','waves2Texture');if(!globalCanvas){curtains=create_canvas(parent);if(curtains)jQuery(elm).data('curtains',curtains);}
if(curtains){if(!permanentDrawing)curtains.disableDrawing();plane=curtains.addPlane(parent,get_params(elm,img,parent));if(plane){jQuery(elm).data('curtains-plane',plane);handle_plane(plane);}}}
...
parent=img.parentNode;if(img_all.length==1){parent.classList.add('trx_addons_image_effects_holder');}
img.setAttribute('data-sampler','rippleTexture');if(!globalCanvas){curtains=create_canvas(parent);if(curtains)jQuery(elm).data('curtains',curtains);}
if(curtains){if(!permanentDrawing)curtains.disableDrawing();plane=curtains.addPlane(parent,get_params(elm,img,parent));if(plane){jQuery(elm).data('curtains-plane',plane);handle_plane(plane);}}}
function get_params(){var vs=`
...
parent=img.parentNode;if(img_all.length==1){parent.classList.add('trx_addons_image_effects_holder');}
img.setAttribute('data-sampler','smudgeTexture');if(!globalCanvas){curtains=create_canvas(parent);if(curtains)jQuery(elm).data('curtains',curtains);}
if(curtains){if(!permanentDrawing)curtains.disableDrawing();plane=curtains.addPlane(parent,get_params(elm,img,parent));if(plane){jQuery(elm).data('curtains-plane',plane);handle_plane(plane);}}}
function get_params(elm,img,parent){var vs=`
				#ifdef GL_ES
					precision mediump float;
...
parent=img.parentNode;if(img_all.length==1){parent.classList.add('trx_addons_image_effects_holder');}
img.setAttribute('data-sampler','tintTexture');if(!globalCanvas){curtains=create_canvas(parent);if(curtains)jQuery(elm).data('curtains',curtains);}
if(curtains){if(!permanentDrawing)curtains.disableDrawing();plane=curtains.addPlane(parent,get_params(elm,img,parent));if(plane){jQuery(elm).data('curtains-plane',plane);handle_plane(plane);}}}
function get_params(elm,img,parent){var vs=`
				#ifdef GL_ES
					precision mediump float;
...
parent=img.parentNode;if(img_all.length==1){parent.classList.add('trx_addons_image_effects_holder');}
img.setAttribute('data-sampler','scrollerTexture');if(!globalCanvas){curtains=create_canvas(parent);if(curtains)jQuery(elm).data('curtains',curtains);}
...
function handle_plane(plane){plane.onReady(function(){plane.tween=null;plane.tweenScale=null;var scrollBusy=false,clear_scroll_busy=trx_addons_throttle(function(){scrollBusy=false;},300,true);function scroll_get(){return(typeof window.scrollY!='undefined'?window.scrollY:window.pageYOffset)||0;}
function scroll_start(e){clear_scroll_busy();if(scrollY<0){scrollY=scroll_get();}else if(!scrollDirection){var scrollNew=scroll_get();scrollDirection=scrollNew>scrollY?1:-1;scrollBusy=true;change_scroll(1);}}
window.addEventListener("scroll",scroll_start);function mouse_move(e){if(!mouseIn){mouse_enter();}}
function mouse_enter(){if(!mouseIn){mouseIn=true;change_scale(1);}}
function mouse_leave(){if(mouseIn){mouseIn=false;change_scale(0);}}
